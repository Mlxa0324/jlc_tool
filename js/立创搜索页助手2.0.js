...
(async function () {
    ...
    // 获取顶级的行元素
    const getParentRow = (that) => $(that).closest('.line-box');
    ...
    const getAllCoupon = async () => {
        ...
        if (couponData) {
            if ([...allOneCouponMap.keys()].length == 0) {
    ...
    const multiFilterBrand = async (isNew) => {
        ...
        $(`.${elementStr}`).each(function () {
            // 品牌名称
            const brandNameOrigin = brandNameProcess($(this).text().trim().trim());
    ...
    function appendSearchTbBtn() {
        if ($('.searchTaobao_').length === 0) {
            ...
            $('.searchTaobao_:not([addedClickHandler])').attr('addedClickHandler', true).on('click', function (params) {
                let searchArrVals = [];
                const $parents = getParentRow(this);
                // 阻容处理、其他元件处理
    ...
    function appendLeftRowBtns() {
        const rows = getSearchRow();
